...
- **Auth follow-ups (not built),** agreed order: follower email digest (Resend) → Google OAuth + account-linking. (Note moderation + dev email-to-console shipped 2026-07-05.) Next hardening steps in `docs/environments.md`.
- **Signed-in/out UX distinction** (Nico, 2026-07-05): signing out produces no noticeable shift — make the signed-in state deliberately different (account chip in nav, visibly unlocked note form/follow button vs. teased).
