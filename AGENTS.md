...
- **Cleanup:** delete the merged `origin/docs/vibe-coding-rewrite` branch. Remove dead `HISTORY_TURSO_*` vars from the env template — nothing reads them (the feed pulls history over HTTP from `prompt-labs.org/api/public_history`, see `lib/history.ts`, not Turso).
